...
class EntropyModel(tf.keras.layers.Layer):
  ...
  def __init__(self, tail_mass=2 ** -8, likelihood_bound=1e-9,
               range_coder_precision=16, noise=True, **kwargs):
    """Initializer.
  ...
  def call(self, inputs, training):
    ...
    if not training:
      quantize_type = "dequantize"
    elif self._quant_noise:
      quantize_type = "noise"
    else:
      quantize_type = "quant_with_grad"
    outputs = self._quantize(inputs, quantize_type)
...
class EntropyBottleneck(EntropyModel):
  ...
  def __init__(self, init_scale=10, filters=(3, 3, 3),
               data_format="channels_last", noise=True, **kwargs):
    """Initializer.
  ...
  def _quantize(self, inputs, mode):
    ...
    if mode == "quant_with_grad":
      non_differentiable = tf.round(outputs)
      outputs = outputs - (tf.stop_gradient(outputs) - non_differentiable)
